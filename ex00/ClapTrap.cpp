...
void	ClapTrap::attack(std::string const& target) {
	if (!this->isAlive() || this->m_energyPoints < 1) {
		std::cerr << this->m_name << " can't attack " << target << ": ";
		this->printStatus();
		return ;
	}
	if (target.empty()) {
		std::cerr << this->m_name << " can't attack: No target" << std::endl;
		return ;
	}
	std::cout
...
void	ClapTrap::takeDamage(unsigned int amount) {
	if (!this->isAlive()) {
		std::cerr << this->m_name << " can't take damage: ";
		this->printStatus();
		return ;
	}
	if (amount > this->m_hitPoints) {
		unsigned int const	diff = amount - this->m_hitPoints;
...
void	ClapTrap::beRepaired(unsigned int amount) {
	if (!this->isAlive() || this->m_energyPoints < 1) {
		std::cerr << this->m_name << " can't be repaired: ";
		this->printStatus();
		return ;
	}
	std::cout
